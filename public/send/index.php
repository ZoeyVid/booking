...
<div style="text-align: center;">
    ...
    <?php if (!array_key_exists("pswd", $_POST) || !array_key_exists("h-captcha-response", $_POST) || !array_key_exists("who", $_POST) || !array_key_exists("subject", $_POST) || !array_key_exists("html-mode", $_POST) || ($_POST["html-mode"] === "no" && !array_key_exists("plain-text", $_POST)) || ($_POST["html-mode"] === "yes" && !array_key_exists("html-text", $_POST)) || (array_key_exists("keep-plain", $_POST) && $_POST["keep-plain"] === "yes" && !array_key_exists("plain-text", $_POST))) { ?>
        <a href="https://github.com/ZoeyVid/booking/blob/main/README.md">Unterstützte Variablen</a><br><br>
        <form method="post" id="form">
            ...
            <fieldset id="html-mode">
                <legend>HTML?</legend>
                <input type="radio" id="html" name="html-mode" value="yes" onchange="document.getElementById('html-text-label').hidden = false; document.getElementById('html-text').hidden = false; document.getElementById('html-text').disabled = false; document.getElementById('keep-plain').hidden = false; document.getElementById('keep-plain').disabled = false;"><label for="html">Ja</label>
                <input type="radio" id="plain" name="html-mode" value="no" onchange="document.getElementById('html-text-label').hidden = true; document.getElementById('html-text').hidden = true; document.getElementById('html-text').disabled = true; document.getElementById('keep-plain').hidden = true; document.getElementById('keep-plain').disabled = true;" checked><label for="plain">Nein</label>
            </fieldset>
            <fieldset id="keep-plain" disabled hidden="hidden">
                <legend>Dennoch Plain zusätzlich senden?</legend>
                <input type="radio" id="no-plain" name="keep-plain" value="yes" onchange="document.getElementById('plain-text-label').hidden = false; document.getElementById('plain-text').hidden = false; document.getElementById('plain-text').disabled = false" checked><label for="no-plain">Ja</label>
                <input type="radio" id="only-html" name="keep-plain" value="no" onchange="document.getElementById('plain-text-label').hidden = true; document.getElementById('plain-text').hidden = true; document.getElementById('plain-text').disabled = true"><label for="only-html">Nein</label>
            </fieldset>
            ...
            <label for="plain-text" id="plain-text-label"><br>Plain Text:<br></label><textarea id="plain-text" name="plain-text" form="form" style="width: calc(200%/3);" required></textarea><br>
                ...
                if (($ennotify && $mail->send()) || !$ennotify) {
                    ...
                    if ($_POST["who"] === "cf" || $_POST["who"] === "ncf" || $_POST["who"] === "all") {
                        while ($row = $results->fetchArray(SQLITE3_ASSOC)) {
                            $plain = str_replace("?email?", $row["email"], $_POST["plain-text"]);
                            $plain = str_replace("?pin?", $row["pin"], $plain);
                            $plain = str_replace("?vn?", $row["vn"], $plain);
                            $plain = str_replace("?nn?", $row["nn"], $plain);
                            $plain = str_replace("?bookinglink?", "https://" . $host . "?bookingtoken=" . $row["bookingtoken"], $plain);
                            $plain = str_replace("?stornolink?", "https://" . $host . "?stornotoken=" . $row["stornotoken"], $plain);
                            if ($enyear) {
                                $plain = str_replace("?year?", $row["year"], $plain);
                            }

                            $html = str_replace("?email?", $row["email"], $_POST["html-text"]);
                            $html = str_replace("?pin?", $row["pin"], $html);
                            $html = str_replace("?vn?", $row["vn"], $html);
                            ...
                            $mail->Subject = "[" . $mail_name . "] " . $_POST["subject"];
                            $mail->clearAddresses();
                            $mail->addAddress($row["email"], $row["vn"] . " " . $row["nn"]);

                            if ($_POST["html-mode"] === "yes") {
                                $mail->isHTML();
                                $mail->Body = $html;
                        ...
                        $msg = "E-Mails erfolgreich versendet: " . $s . ", fehlgeschlagen: " . $f . ", insgesamt: " . $g;
                    } else {
                        $msg = "Formular fehlerhaft!" . $err;
                    }
                } else {
